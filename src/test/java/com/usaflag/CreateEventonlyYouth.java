...
/**
 * Create Event Test for USA Flag Application - YOUTH REGISTRATION ONLY
 * This test class handles admin login and event creation functionality
 * with Youth Registration only (no Adult Registration).
 */
public class CreateEventonlyYouth {
    ...
    public void testDetails() {
        try {
            ...
            String logoPath = "C:\\Users\\User\\Desktop\\profile-pics\\logo\\imagesa.png";
            ...
            // Click Preview button - scroll and click using multiple fallback approaches
            System.out.println("⚠️ Looking for Preview button...");
            try {
                WebElement previewButton = null;
                boolean previewClicked = false;
                
                // Try multiple approaches to find the Preview button with scrolling
                for (int attempt = 1; attempt <= 10; attempt++) {
                    try {
                        System.out.println("⚠️ Attempt " + attempt + " to find Preview button...");
                        
                        // Try Approach 1: Find by SVG path (most specific)
                        try {
                            previewButton = driver.findElement(By.xpath("//button[.//svg//path[@d='M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z']]"));
                            System.out.println("✅ Found Preview button (Approach 1 - SVG path)");
                            break;
                        } catch (Exception e1) {
                            // Try Approach 2: Find by CSS selector (specific path)
                            try {
                                previewButton = driver.findElement(By.cssSelector("body > div:nth-child(2) > div:nth-child(2) > div:nth-child(2) > main:nth-child(1) > div:nth-child(1) > form:nth-child(2) > div:nth-child(2) > button:nth-child(2)"));
                                System.out.println("✅ Found Preview button (Approach 2 - CSS selector)");
                                break;
                            } catch (Exception e2) {
                                // Try Approach 3: Find by data-slot and text
                                try {
                                    previewButton = driver.findElement(By.xpath("//button[@data-slot='button' and contains(., 'Preview')]"));
                                    System.out.println("✅ Found Preview button (Approach 3 - data-slot)");
                                    break;
                                } catch (Exception e3) {
                                    // Try Approach 4: Find by text "Preview" 
                                    try {
                                        previewButton = driver.findElement(By.xpath("//button[normalize-space()='Preview']"));
                                        System.out.println("✅ Found Preview button (Approach 4 - text match)");
                                        break;
                                    } catch (Exception e4) {
                                        // Try Approach 5: Find by bg-dark-blue class
                                        try {
                                            previewButton = driver.findElement(By.xpath("//button[contains(@class, 'bg-dark-blue') and contains(., 'Preview')]"));
                                            System.out.println("✅ Found Preview button (Approach 5 - bg-dark-blue)");
                                            break;
                                        } catch (Exception e5) {
                                            // Scroll down to potentially reveal the button
                ...
                if (previewButton != null) {
                    ...
                    System.out.println("📊 Preview button info:");
                    System.out.println("   - Displayed: " + previewButton.isDisplayed());
                    System.out.println("   - Enabled: " + previewButton.isEnabled());
                    System.out.println("   - Text: " + previewButton.getText());
                    System.out.println("   - Location: " + previewButton.getLocation());
                    System.out.println("   - Size: " + previewButton.getSize());
                    
                    // Multiple attempts to click the button
                    for (int clickAttempt = 1; clickAttempt <= 3; clickAttempt++) {
                        try {
                            System.out.println("⚠️ Click attempt " + clickAttempt + " for Preview button...");
                            
                            // Scroll to the very bottom first to ensure button is visible
                            ((JavascriptExecutor) driver).executeScript("window.scrollTo(0, document.body.scrollHeight);");
                            Thread.sleep(800);
                            
                            // Check if button is displayed
                            if (!previewButton.isDisplayed()) {
                                System.out.println("⚠️ Button not visible, scrolling to element...");
                                ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'instant', block: 'center'});", previewButton);
                                Thread.sleep(800);
                            }
                            
                            // Scroll button to view
                            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'instant', block: 'center'});", previewButton);
                            Thread.sleep(1000);
                            ...
                            try {
                                previewButton.click();
                                System.out.println("✅ Preview button clicked successfully (regular click, attempt " + clickAttempt + ")");
                                previewClicked = true;
                                break;
                            } catch (Exception e1) {
                                ...
                                try {
                                    ...
                                    System.out.println("✅ Preview button clicked successfully (JavaScript click, attempt " + clickAttempt + ")");
                                    previewClicked = true;
                                    break;
                                } catch (Exception e2) {
                                    System.out.println("⚠️ JavaScript click failed, trying actions...");
                                    
                                    // Try Actions class click
                                    try {
                                        org.openqa.selenium.interactions.Actions actions = new org.openqa.selenium.interactions.Actions(driver);
                                        actions.moveToElement(previewButton).click().build().perform();
                                        System.out.println("✅ Preview button clicked successfully (Actions click, attempt " + clickAttempt + ")");
                                        previewClicked = true;
                                        break;
                                    } catch (Exception e3) {
                                        System.out.println("⚠️ Actions click failed, trying coordinates click...");
                                        
                                        // Try coordinates-based click as last resort
                                        try {
                                            int x = previewButton.getLocation().x + previewButton.getSize().width / 2;
                                            int y = previewButton.getLocation().y + previewButton.getSize().height / 2;
                                            System.out.println("📊 Attempting click at coordinates: (" + x + ", " + y + ")");
                                            ((JavascriptExecutor) driver).executeScript("document.elementFromPoint(" + x + ", " + y + ").click();");
                                            System.out.println("✅ Preview button clicked successfully (coordinates click, attempt " + clickAttempt + ")");
                                            previewClicked = true;
                                            break;
                                        } catch (Exception e4) {
                                            System.out.println("⚠️ Coordinates click failed on attempt " + clickAttempt);
                                            if (clickAttempt < 3) {
                                                Thread.sleep(500);
                                            }
                                        }
                            ...
                            System.out.println("❌ Click attempt " + clickAttempt + " failed: " + e.getMessage());
                        }
                    }
                    ...
                    if (previewClicked) {
                        ...
                        System.out.println("❌ All click attempts failed for Preview button");
                    }
                } else {
                    System.out.println("❌ Preview button not found after scrolling attempts - continuing without clicking");
                }
            } catch (Exception e) {
                System.out.println("⚠️ Failed to click Preview button: " + e.getMessage());
            }
    ...
    public void testPayment() throws InterruptedException {
        try {
            System.out.println("💳 Starting PAYMENT SECTION - YOUTH ONLY...");
            ...
            // ============================================
            // CLICK YOUTH REGISTRATION CHECKBOX
            // ============================================
            
            System.out.println("⚠️ Looking for Youth Registration checkbox...");
            
            // Find the Youth Registration checkbox button
            WebElement youthRegCheckbox = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//label[contains(text(), 'Youth Registration')]/preceding-sibling::button[@role='checkbox']")));
            
            // Scroll to checkbox
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'instant', block: 'center'});", youthRegCheckbox);
            Thread.sleep(500);
            
            // Check current state
            String ariaChecked = youthRegCheckbox.getAttribute("aria-checked");
            System.out.println("📋 Youth Registration checkbox state: " + ariaChecked);
            
            // Click checkbox to enable Youth Registration (if not already checked)
            if (!"true".equals(ariaChecked)) {
                ((JavascriptExecutor) driver).executeScript("arguments[0].click();", youthRegCheckbox);
                System.out.println("✅ Clicked Youth Registration checkbox to enable it");
                Thread.sleep(1000);
            } else {
                System.out.println("✅ Youth Registration checkbox already enabled");
            }
            
            // Wait for Youth Registration section to appear
            Thread.sleep(2000);
            
            // Scroll to "Youth Registration" visible text
            WebElement youthRegistrationText = wait.until(ExpectedConditions.visibilityOfElementLocated(
                By.xpath("//*[normalize-space(text())='Youth Registration']")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'instant', block: 'center'});", youthRegistrationText);
            System.out.println("✅ Scrolled to 'Youth Registration' section");
            Thread.sleep(500);
            
            System.out.println("✅ Skipping Adult Registration - Youth Only mode");
            ...
            try {
                // ============================================================
                // YOUTH REGISTRATION - YOUTH ONLY VERSION
                // ============================================================
                ...
                // Since only Youth Registration is enabled, get the first (and only) Style button
                WebElement youthStyle = wait.until(ExpectedConditions.elementToBeClickable(
                    By.xpath("//label[contains(text(), 'Style')]/following-sibling::button")));
                
                // WebElement youthStyle = styleButtons.get(0); // ONLY Youth fields are showing
                ...
                // Since only Youth Registration is enabled, get the first (and only) Category button
                WebElement youthCategory = wait.until(ExpectedConditions.elementToBeClickable(
                ...
                // Wait for Division button to be present (only Youth fields are showing)
                WebDriverWait divisionWait = new WebDriverWait(driver, Duration.ofSeconds(10));
                WebElement youthDivision = divisionWait.until(ExpectedConditions.presenceOfElementLocated(
                    By.xpath("//label[contains(text(), 'Division')]/following-sibling::button[@role='combobox']")));
                System.out.println("🔍 Found Youth Division button");
                ...
                // Find and click "Amateur" (since only Youth fields are showing, we can use the first)
                WebElement youthAmateurOption = wait.until(ExpectedConditions.elementToBeClickable(
                ...
                // Since only Youth Registration is enabled, get the first (and only) Waitlist button
                WebElement youthWaitlist = wait.until(ExpectedConditions.elementToBeClickable(
                ...
                // Find and click "Yes" (since only Youth fields are showing, we can use the first)
                WebElement youthYesOption = wait.until(ExpectedConditions.elementToBeClickable(
            ...
            // ============================================
            // ENSURE YOUTH REGISTRATION IS COMPLETE BEFORE NEXT BUTTON
            // ============================================
    ...
    public void testAddOns() {
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (addVendorButton != null) {
                        ...
                        try {
                            ...
                            System.out.println("✅ Clicked 'Add Vendor/Sponsor' button (regular click)");
                        } catch (Exception clickEx) {
                            // Fallback to Actions click if regular click fails
                            try {
                                org.openqa.selenium.interactions.Actions actions = new org.openqa.selenium.interactions.Actions(driver);
                                actions.moveToElement(addVendorButton).click().build().perform();
                                System.out.println("✅ Clicked 'Add Vendor/Sponsor' button (Actions click)");
                            } catch (Exception actionsEx) {
                                System.out.println("⚠️ Failed to click Add Vendor/Sponsor button: " + actionsEx.getMessage());
                            }
                        }
                ...
                // Click Preview button after Add Vendor/Sponsor
                System.out.println("⚠️ Clicking Preview button after Add Vendor/Sponsor...");
                try {
                    WebElement previewButtonFinal = null;
                    
                    // Try multiple approaches to find the Preview button
                    try {
                        // Approach 1: Find by SVG path
                        previewButtonFinal = driver.findElement(By.xpath("//button[.//svg//path[@d='M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z']]"));
                        System.out.println("✅ Found Preview button (SVG path)");
                    } catch (Exception e1) {
                        try {
                            // Approach 2: Find by text "Preview"
                            previewButtonFinal = driver.findElement(By.xpath("//button[normalize-space()='Preview']"));
                            System.out.println("✅ Found Preview button (text)");
                        } catch (Exception e2) {
                            System.out.println("⚠️ Preview button not found");
                        }
                    }
                    
                    if (previewButtonFinal != null) {
                        // Scroll to button
                        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'});", previewButtonFinal);
                        Thread.sleep(1000);
                        
                        // Click the button
                        try {
                            previewButtonFinal.click();
                            System.out.println("✅ Preview button clicked successfully!");
                        } catch (Exception clickEx) {
                            try {
                                org.openqa.selenium.interactions.Actions actions = new org.openqa.selenium.interactions.Actions(driver);
                                actions.moveToElement(previewButtonFinal).click().build().perform();
                                System.out.println("✅ Preview button clicked successfully (Actions)!");
                            } catch (Exception actionsEx) {
                                System.out.println("⚠️ Failed to click Preview button: " + actionsEx.getMessage());
                            }
                        }
                    ...
                    System.out.println("⚠️ Exception while clicking Preview button: " + e.getMessage());
                }
